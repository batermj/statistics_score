...
#好变量，中等变量，差变量,或疑惑变量列表存储器
list_goodInformative_variables=[]
list_mediumInformative_variables=[]
list_lowInformative_variables=[]
list_goodOrSuspicious_variables=[]
#被选中的变量列表
list_choose=[]
...
for i in range(len(list_columns)):
    ...
    if status=="A":
        list_goodInformative_variables.append(columnName)
    if status=="B":
        list_mediumInformative_variables.append(columnName)
    if status=="C":
        list_lowInformative_variables.append(columnName)
    if status=="D":
        list_goodOrSuspicious_variables.append(columnName)


#生成表格，包含价值量高的变量
list_choose=list_goodOrSuspicious_variables+list_goodInformative_variables+list_mediumInformative_variables
